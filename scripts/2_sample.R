...
################################################################################
# Subset the sample from the population based on a minimum traffic threshold   #
################################################################################
...
distm(c(0, median(df_pop$lat[!rev(duplicated(rev(df_pop$icao)))])), c(0, 0), fun = distHaversine) / 1000 # To the population's median
distm(c(0, mean(df_pop$lat[!rev(duplicated(rev(df_pop$icao)))])), c(0, 0), fun = distHaversine) / 1000   # To the population's mean
distm(c(0, median(df_smp$lat[!rev(duplicated(rev(df_smp$icao)))])), c(0, 0), fun = distHaversine) / 1000 # To the sample's median
distm(c(0, mean(df_smp$lat[!rev(duplicated(rev(df_smp$icao)))])), c(0, 0), fun = distHaversine) / 1000   # To the sample's mean

# Calculate the distance from the median to the mean
distm(c(0, median(df_pop$lat[!rev(duplicated(rev(df_pop$icao)))])), c(0, mean(df_pop$lat[!rev(duplicated(rev(df_pop$icao)))])), fun = distHaversine) / 1000 # Population
distm(c(0, median(df_smp$lat[!rev(duplicated(rev(df_smp$icao)))])), c(0, mean(df_smp$lat[!rev(duplicated(rev(df_smp$icao)))])), fun = distHaversine) / 1000 # Sample
...
df_pop_binned <- df_pop[!duplicated(df_pop$icao),] %>% # Summarize observations to just one per airport
  mutate(bin = cut(traffic, breaks = breaks, labels = labels, include.lowest = TRUE, right = FALSE)) %>%
  mutate(geo = cut(lat, breaks = geo_zones, labels = geo_labels, include.lowest = TRUE, right = FALSE))

# Bin the sample airports (not runways) by passenger traffic and geographical zones
df_smp_binned <- df_smp[!duplicated(df_smp$icao),] %>% # Summarize observations to just one per airport
  mutate(bin = cut(traffic, breaks = breaks, labels = labels, include.lowest = TRUE, right = FALSE)) %>%
  mutate(geo = cut(lat, breaks = geo_zones, labels = geo_labels, include.lowest = TRUE, right = FALSE))
...
df_pop_binned %>%
  ...
  mutate(per = percent(n / sum(df_pop[!duplicated(df_pop$icao),]$traffic), accuracy = .01)) # Percentage

# Sum the sample traffic by geographical zone
df_smp_binned %>%
  group_by(geo) %>%
  summarize(n = sum(traffic)) %>% # Sum
  mutate(per = percent(n / sum(df_smp[!duplicated(df_smp$icao),]$traffic), accuracy = .01)) # Percentage
...
smp_world <- ggplot() +
  ...
  geom_point(data = df_pop[!duplicated(df_pop$icao),], mapping = aes(x = lon, y = lat), color = "black", shape = 20, size = 1.5) + # Draw the population airport points
  geom_point(data = df_smp[!duplicated(df_smp$icao),], mapping = aes(x = lon, y = lat), color = "purple", shape = 20, size = 1.5) + # Draw the sample airport points
  geom_hline(aes(yintercept = df_pop$lat[which.max(df_pop$lat)]), color = "black") + # Draw the population's northernmost parallel
  geom_hline(aes(yintercept = df_smp$lat[which.max(df_smp$lat)]), color = "purple") + # Draw the sample's northernmost parallel
  geom_hline(aes(yintercept = df_pop$lat[which.min(df_pop$lat)]), color = "black") + # Draw the population's southernmost parallel
  geom_hline(aes(yintercept = df_smp$lat[which.min(df_smp$lat)]), color = "purple") + # Draw the sample's southernmost parallel
  geom_hline(aes(yintercept = mean(df_pop[!duplicated(df_pop$icao),]$lat)), color = "black") +
  geom_hline(aes(yintercept = mean(df_smp[!duplicated(df_smp$icao),]$lat)), color = "purple") +
  geom_hline(aes(yintercept = median(df_pop[!duplicated(df_pop$icao),]$lat)), color = "black") +
  geom_hline(aes(yintercept = median(df_smp[!duplicated(df_smp$icao),]$lat)), color = "purple") +
...
smp_hist_apt_cnt <- ggplot() +
  geom_histogram(mapping = aes(x = df_pop[!duplicated(df_pop$icao),]$lat), fill="black", alpha=0.5, binwidth = 10, na.rm = TRUE) +
  geom_histogram(mapping = aes(x = df_smp[!duplicated(df_smp$icao),]$lat), fill="black", alpha=0.5, binwidth = 10, na.rm = TRUE) +
...
smp_hist_apt_tra <- ggplot() +
  geom_histogram(mapping = aes(x = df_pop[!duplicated(df_pop$icao),]$lat, weight = df_pop[!duplicated(df_pop$icao),]$traffic), fill="black", alpha=0.5, binwidth = 10, na.rm = TRUE) +
  geom_histogram(mapping = aes(x = df_smp[!duplicated(df_smp$icao),]$lat, weight = df_smp[!duplicated(df_smp$icao),]$traffic), fill="black", alpha=0.5, binwidth = 10, na.rm = TRUE) +
  scale_x_continuous(name = "Latitude", breaks = seq(-90, 90, 10), limits = c(-90, 90)) +
...
df_kgc_pop <- df_pop[!duplicated(df_pop$icao),c("icao", "lon", "lat", "traffic")] %>% # Select unique airport locations and coordinates from the population
  mutate(rndCoord.lon = RoundCoordinates(lon, res = resolution, latlong = "lon")) %>% # Round longitudes to the nearest KGC grid coordinate
  mutate(rndCoord.lat = RoundCoordinates(lat, res = resolution, latlong = "lat")) # Round latitudes to the nearest KGC grid coordinate
...
df_kgc_pop <- df_kgc_pop %>%
  group_by(kgc) %>%
  dplyr::summarize(pop.airports = n(), pop.traffic = sum(traffic)) # Count. We have to call out the dplyr package explicitly because summarize() also exists in plyr and is being masked

# Prepare the sample data
df_kgc_smp <- df_smp[!duplicated(df_smp$icao),c("icao", "lon", "lat", "traffic")] %>% # Select unique airport locations and coordinates from the sample
  mutate(rndCoord.lon = RoundCoordinates(lon, res = resolution, latlong = "lon")) %>% # Round longitudes to the nearest KGC grid coordinate
  mutate(rndCoord.lat = RoundCoordinates(lat, res = resolution, latlong = "lat")) # Round latitudes to the nearest KGC grid coordinate
...
df_kgc_smp <- df_kgc_smp %>%
  group_by(kgc) %>%
  dplyr::summarize(smp.airports = n(), smp.traffic = sum(traffic)) # Count. We have to call out the dplyr package explicitly because summarize() also exists in plyr and is being masked
...
smp_apt_dist_cz <- ggplot(data = df_kgc) +
...
smp_tra_dist_cz <- ggplot(data = df_kgc) +
