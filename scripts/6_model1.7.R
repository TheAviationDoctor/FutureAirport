...
fn_todr <- function(DT) {
  ...
  # Calculate the regulatory buffer assuming a dry runway and no clearway
  dis_reg <- (dis_gnd + dis_air) * (sim$tod_mul - 1)
  
  # Assemble the takeoff distance required in m
  # todr <- dis_gnd + dis_air + dis_reg
  ...
  # fwrite(V, file = "V.csv", append = TRUE)
  fwrite(X, file = "X.csv", append = TRUE)
  
  # Return the TODR rounded up to the nearest integer
  return(todr)
