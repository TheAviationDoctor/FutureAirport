...
filepath <- "data/population/inputs"
filename <- "runways.csv"
df_rwy <- read.csv(file = file.path(filepath, filename, fsep = "/"), header = TRUE, na.strings = c(0, "NULL"), col.names = c("lido.id", "lido.area", "lido.icao", "lido.country", "lido.rwy", "lido.len", "lido.lat", "lido.lon", "lido.toda", "lido.toda.night", "lido.unit"), colClasses = c("integer", "factor", "character", "factor", "character", "integer", "character", "character", "integer", "integer", "factor"))

# Describe the data
str(df_rwy)

# Retain only the variables of interest
df_rwy <- subset(df_rwy, select = c("lido.icao", "lido.rwy", "lido.len", "lido.toda"))

# Count missing TODAs
sum(is.na(df_rwy$lido.toda))

# Remove the missing TODAs
df_rwy <- na.omit(df_rwy)

# Count unique observations (runways)
length(unique(df_rwy$lido.icao))

# Calculate mean count of observations (runways) per airport
nrow(df_rwy) / length(unique(df_rwy$lido.icao))
...
df_tra <- read.csv(file = file.path(filepath, filename, fsep = "/"), header = TRUE, col.names = c("iata.code", "iata.traffic"), colClasses = c("character", "integer"))

# Describe the data
str(df_tra)

# Sum up the total traffic
sum(df_tra$iata.traffic)

# Describe the traffic variable
summary(df_tra$iata.traffic)
...
df_tra_binned <- df_tra %>%
  mutate(bin = cut(df_tra$iata.traffic, breaks = breaks, labels = labels, include.lowest = TRUE, right = FALSE)) %>% # Add the bin to each airport
  group_by(bin) %>% # Group the airports by bin
  summarize(airports = n(), traffic = sum(iata.traffic)) %>% # Summarize the airport count and traffic by bin
...
ggplot(data = df_tra, mapping = aes(x = iata.traffic)) +
  geom_density(alpha = .75, fill = "lightgray") +
  geom_vline(xintercept = mean(df_tra$iata.traffic), color = "black") +
  geom_vline(xintercept = median(df_tra$iata.traffic), color = "black", linetype = "dashed") +
...
df_geo <- read.csv(file = file.path(filepath, filename, fsep = "/"), header = TRUE, col.names = c("our.id", "our.ident", "our.type", "our.name", "our.lat", "our.lon", "our.elev", "our.cont", "our.country", "our.region", "our.city", "our.sched", "our.icao", "our.iata", "our.local", "our.website", "our.wiki", "our.keywords"), colClasses = c("integer", "character", "factor", "character", "numeric", "numeric", "numeric", "factor", "factor", "factor", "character", "factor", "character", "character", "character", "character", "character", "character"))

# Describe the data
str(df_geo)

# Keep only non-closed airports with an IATA code and count the resulting observations
df_geo <- subset(df_geo, our.type %in% c("small_airport", "medium_airport", "large_airport") & our.icao != "" & nchar(our.iata) == 3, select = c("our.name", "our.lat", "our.lon", "our.icao", "our.iata"))
nrow(df_geo)
...
count(df_apt[!complete.cases(df_apt$our.icao),])

# Describe the larger airports (>= 10^6 passengers) missing an ICAO code
str(df_apt$iata.code[!complete.cases(df_apt$our.icao) & df_apt$iata.traffic >= 10^6])

# Describe the smaller airports (< 10^6 passengers) missing an ICAO code
missing_small <- df_apt[!complete.cases(df_apt$our.icao) & df_apt$iata.traffic < 10^6,]
str(missing_small$iata.code) # Describe
sum(missing_small$iata.traffic) # Calculate traffic
sum(missing_small$iata.traffic) / sum(df_tra$iata.traffic) * 100 # Calculate traffic share

# Impute or remove the airports missing an ICAO code from the upstream traffic dataset
df_tra$iata.traffic[df_tra$iata.code == "BER"] <- df_tra$iata.traffic[df_tra$iata.code == "SXF"] + df_tra$iata.traffic[df_tra$iata.code == "TXL"] # Manually transfer traffic from two large airports to another that absorbed them
df_tra <- subset(df_tra, !(iata.code %in% c("SXF", "TXL"))) # Remove those two larger airports
df_tra$iata.code[df_tra$iata.code == "TSE"] <- "NQZ" # Manually rename one larger airport whose IATA code changed
df_tra <- subset(df_tra, !(iata.code %in% missing_small$iata.code)) # Remove the smaller airports
...
df_apt[duplicated(df_apt$iata.code) | duplicated(df_apt$iata.code, fromLast = TRUE),]

# Remove duplicates and count the resulting cases
df_apt <- subset(df_apt, our.name != "Liuting Airport" & our.name != "Dewadaru - Kemujan Island" & our.name != "Yibin Caiba Airport") # Remove three occurrences of false duplicates (i.e. two different airports with the same IATA code)
df_apt <- df_apt[!rev(duplicated(rev(df_apt$iata.code))),] # Remove first occurrence only of strict duplicates (i.e. keep one of each)
nrow(df_apt) # Count

# Order the population by decreasing traffic size and reset the row index
df_apt <- df_apt[order(df_apt$iata.traffic, decreasing = TRUE),] # Order
...
df_pop <- merge(x = df_apt, y = df_rwy, by.x = "our.icao", by.y = "lido.icao", all.x = TRUE)

# Describe the data
str(df_pop)

# Check for and remove missing runways
count(df_pop[!complete.cases(df_pop$lido.rwy),])
df_pop <- subset(df_pop, complete.cases(df_pop$lido.rwy))
...
df_pop_unique <- df_pop %>%
  mutate(rwy.recip = if_else( # Calculate reciprocal heading for each runway
    parse_number(lido.rwy) <= 18, # 180 degrees
    paste("RW", formatC(parse_number(lido.rwy) + 18, width = 2, format = "d", flag = "0"), case_when(str_sub(lido.rwy, -1, -1) == "L" ~ "R", str_sub(lido.rwy, -1, -1) == "R" ~ "L", str_sub(lido.rwy, -1, -1) == "C" ~ "C", TRUE ~ ""), sep = ""),
    paste("RW", formatC(parse_number(lido.rwy) - 18, width = 2, format = "d", flag = "0"), case_when(str_sub(lido.rwy, -1, -1) == "L" ~ "R", str_sub(lido.rwy, -1, -1) == "R" ~ "L", str_sub(lido.rwy, -1, -1) == "C" ~ "C", TRUE ~ ""), sep = "")
  )) %>%
  mutate(rwy.concat = pmap_chr(list(lido.rwy, rwy.recip), ~ paste(sort(c(...)), collapse = "-"))) %>%
  unite("rwy.unique", c("our.icao", "rwy.concat", "lido.toda"), sep = "-", remove = FALSE) %>%
  arrange(desc(iata.traffic), rwy.unique) %>%
  select(rwy.unique)
...
summary(df_pop$lido.len)
summary(df_pop$lido.toda)

# Order the merged dataset by decreasing traffic size and ICAO code
df_pop <- df_pop[order(df_pop$iata.traffic, df_pop$our.icao, decreasing = TRUE), ]
...
write.csv(df_pop, file = file.path(filepath, filename, fsep = "/"), row.names = FALSE)
