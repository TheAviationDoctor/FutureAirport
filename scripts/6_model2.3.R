...
# S_total    = wing surface area (incl. extended flaps) in m²
...
fn_Vlof <- function(DT) {
  ...
  Vlof <- sqrt(
    (2 * DT[, m] * sim$g) /
    (DT[, rho] * DT[, S_total] * DT[, cL] * sim$clmax_cllof)
  )
...
fn_dis_gnd <- function(DT) {
  ...
  # ============================================================================
  # 2 Calculate the propulsive force F in N
  ...
  # ============================================================================
  # 3 Calculate the acceleration in m/s² up to liftoff
  # Adapted from Blake (2009).
  # ============================================================================
  ...
  # Vectorize the wing surface area (incl. extended flaps) in m²
  S_total <- rep(DT[, S_total], each = sim$int)
  ...
  a <- (sim$g / W) * (Frto - (sim$mu * W) - (cD - sim$mu * cL) * q * S_total -
    (W * sin(sim$theta)))
  ...
  # ============================================================================
  # 4 Increment the horizontal takeoff distances in m
  # Adapted from Blake (2009).
  # ============================================================================
  ...
  # ============================================================================
  # 5 Assemble the takeoff distance required in m
  ...
  #   "Vtas (m/s)"  = round(Vtas, digits = 1L),
  #   "Vgnd (m/s)"  = round(Vgnd, digits = 1L),
  #   "Vsnd (m/s)"  = round(Vsnd, digits = 0L),
  #   "Vmach (m/s)" = round(Vmach, digits = 2L),
  #   "rho (kg/m3)" = round(rho, digits = 3L),
  #   "dP"          = round(dP, digits = 1L),
  #   "q (N/m2)"    = round(q, digits = 0L),
  #   "bpr"         = bpr,
  #   "n"           = n,
  #   "A"           = round(A, digits = 1L),
  #   "G0"          = round(G0, digits = 1L),
  #   "X"           = round(X, digits = 1L),
  #   "Z"           = round(Z, digits = 1L),
  #   "tr"          = round(tr, digits = 2L),
  #   "slst (N)"    = slst,
  #   "Fmax (N)"    = round(Fmax, digits = 0L),
  #   "rto (%)"     = rto,
  #   "Frto (N)"    = round(Frto, digits = 0L),
  #   "m (kg)"      = m,
  #   "W (N)"       = W,
  #   "S_total (m2)"= round(S_total, digits = 0L),
  #   "cL"          = round(cL, digits = 2L),
  #   "cD"          = round(cD, digits = 2L),
  #   "mu"          = sim$mu,
  #   "mu*W"        = round(sim$mu * W, digits = 0L),
  #   "(cD-mu*cL)"  = round(cD - sim$mu * cL, digits = 2L),
  #   "(cD-mu*cL)qS"= round((cD - sim$mu * cL) * q * S_total, digits = 0L),
  #   "a (m/s2)"    = round(a, digits = 2L),
  #   "a_bar"       = round(a_bar, digits = 2L),
  #   "Vgnd_bar"    = round(Vgnd_bar, digits = 1L),
  #   "Vgnd_int"    = round(Vgnd_int, digits = 1L),
  #   "inc"         = round(inc, digits = 0L),
  #   "cum"         = round(cum, digits = 0L)
  # )
  # View(out)
...
# ==============================================================================
# Define a function to calculate the horizontal distance covered by the aircraft
# during the first-segment climb, in m. Adapted from Filippone (2012, p. 258).
# ==============================================================================

fn_dis_air <- function() {
  
  dis_air <- sim$scrn_hght / tan(sim$climb_angle * pi / 180) * sim$ft_to_m
