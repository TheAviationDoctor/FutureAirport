...
dt_act <- fread(
  file = f$act,
  header = TRUE,
  colClasses = c(rep("factor", 3), rep("integer", 5), rep("numeric", 8)),
  key = "type"
)
...
# ==============================================================================
# 1.5 Calculate the lift-independent drag in takeoff configuration
# Adapted from from Sun et al. (2020).
# ==============================================================================

# Calculate the lift-independent drag attributable to the flaps
set(
  x = dt_tko,
  j = "cD0_flaps",
  value = dt_tko[, lambda_f] * dt_tko[, cfc]^1.38 * dt_tko[, SfS] *
    sin(sim$flaps * pi / 180)^2 # AS PER SUN ET AL (2020) - RADIAN CONVERSION
    # sin(sim$flaps)^2 # FOR TESTING ONLY - NO RADIAN CONVERSION
)

# Calculate the lift-independent drag attributable to the gear
# Adapted from Mair (1992), p. 124
# Sun et al. (2020) uses MTOM twice, but Mair uses actual mass in the first term
# The chosen coefficient is the midpoint between 5.81E-5 (zero flap deflection)
# and 3.16E-5 (maximum flap deflection), in line with a typical takeoff setting.
set(
  x = dt_tko,
  j = "cD0_gear",
  value = dt_tko[, m] * sim$g / dt_tko[, S] # Wing loading in N/m2
    * 4.49E-5                               # K coefficient for flap deflection
    * dt_tko[, maxtom]^-.215                # 
)

# Calculate the total lift-independent drag (fuselage + flaps + gear) at takeoff
set(
  x = dt_tko,
  j = "cD0",
  value = dt_tko[, cD0_clean] + dt_tko[, cD0_flaps] + dt_tko[, cD0_gear]
)
...
# Calculate the lift-induced coefficient k at takeoff - Sun et al. (2020) method
set(x = dt_tko, j = "k", value = 1 / (pi * dt_tko[, ar] * dt_tko[, e_clean]))
...
fn_calibrate <- function(cL, i) {
  ...
  # Calculate the lift-induced drag coefficient - Sun et al. (2020) method
  set(
    x = dt_tko,
    i = i,
    j = "cDi",
  ...
  # Calculate the total drag coefficient - Sun et al. (2020) method
  set(
    x = dt_tko,
    i = i,
    j = "cD",
  ...
  # Calculate the total drag coefficient - using Blake's (2009) parameter
  # This will override the Sun et al. (2020) method
  # set(
  #   x = dt_tko,
  #   i = i,
  #   j = "cD",
  #   # value = dt_tko[i, cD0] + dt_tko[i, cDi]
  #   value = .08 + sim$mu * dt_tko[i, cL]
  # )
  ...
  # Calculate the Blake (2009) coefficient
  set(
    x = dt_tko,
    i = i,
    j = "blake",
    value = dt_tko[i, cD] - sim$mu * dt_tko[i, cL]
  )
...
for (i in seq_len(nrow(dt_tko))) {
  ...
  print(
    paste(
      ...
      dt_tko[i, type],
      "| m =",
      str_pad(dt_tko[i, m], width = 6, side = "left", pad = " "),
      "| cL =",
      format(x = dt_tko[i, cL], digits = 3, nsmall = 3),
      "| cD =",
      format(x = dt_tko[i, cD], digits = 3, nsmall = 3),
      "| cL/cD =",
      format(x = dt_tko[i, cL] / dt_tko[i, cD], digits = 3, nsmall = 3),
      "| cD-mu*cL =",
      format(x = dt_tko[i, cD] - sim$mu * dt_tko[i, cL], digits = 3, nsmall = 3),
      "| Vlof =",
      format(x = dt_tko[i, Vlof], digits = 1, nsmall = 1),
      "| diff_m =",
      format(x = dt_tko[i, diff_m], digits = 0, nsmall = 0),
      sep = " "
    )
  )
...
# Box-plot the Blake coefficient by aircraft type
(ggplot(data = dt_tko[, .(type, blake)],
      aes(x = type, y = blake)) +
    geom_boxplot() +
    labs(x = "Aircraft type", y = "cD - mu * cL") +
    theme_light()) %>%
  ggsave(
    filename = "cal_box_blake.png",
    device = "png",
    path = "plots",
    scale = 1,
    width = 6,
