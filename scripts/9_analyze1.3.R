...
#     "CREATE TABLE IF NOT EXISTS",
#     tolower(tmp$q22),
#     "(year YEAR, exp CHAR(6), zone CHAR(11), type CHAR(4),
#      tko_ok MEDIUMINT, tko_ko MEDIUMINT)",
#     "AS SELECT
#     YEAR(obs) AS year,
#     exp,
#     zone,
#     type,
#     SUM(todr <= toda) AS tko_ok_cnt,
#     SUM(todr > toda) AS tko_ko_cnt
#     FROM",
#     tolower(tmp$tst),
...
# 
# # Convert the climatic zone to a factor
# set(x = dt_q22, j = "zone",  value = as.factor(dt_q22[, zone]))
# 
# # Convert the shared socioeconomic pathway (SSP) to a factor
...
# (
#   ggplot(
#     data    = dt_q22[, sum(tko_ko_cnt) / (sum(tko_ok_cnt) + sum(tko_ko_cnt)),
#                      by = year],
#     mapping = aes(x = year, y = V1)
#   ) +
#     geom_line() +
#     scale_x_continuous(name = "Year") +
#     scale_y_continuous(
#       name   = "Percentage of unsuccessful takeoffs (TODR > TODA)",
#       labels = scales::percent) +
#     theme_light() +
#     theme(axis.title.y = element_blank())
# ) |>
# ggsave(
#   filename = paste("9_q22_tko.png", sep = ""),
#   device   = "png",
#   path     = "plots",
#   scale    = 1,
#   width    = 6,
...
# Convert the year to an interval variable for continuous scaling
set(x = dt_q31, j = "year", value = as.integer(dt_q31[, year]))

# # Convert the climatic zone to a factor
# set(x = dt_q31, j = "zone",  value = as.factor(dt_q31[, zone]))

# Convert the shared socioeconomic pathway (SSP) to a factor
set(x = dt_q31, j = "exp",  value = as.factor(dt_q31[, exp]))

# Convert the aircraft type to a factor
set(x = dt_q31, j = "type",  value = as.factor(dt_q31[, type]))

# Combine narrowbodies and widebodies, respectively
levels(dt_q31$type) <- c(
  "A20n" = "Narrowbody",
  "B39m" = "Narrowbody",
  "A359" = "Widebody",
  "B789" = "Widebody"
...
# Return the local polynomial regression fitting values to smooth the volatility
dt_q31[, paste(c("tko_ok_thr_min_per", "tko_ko_per"), "_loess", sep = "") := lapply(
  X     = .SD,
  FUN   = function(x) {
    predict(loess(formula = x ~ year, span = .75, model = TRUE))
  }
...
# TEST
fwrite(x = dt_q31, file = "dt_q31_percentages.csv")

# Display the summary table
View(
  x     = dt_q31,
  title = "Thrust-reduced takeoffs by group"
)

# Define a function to generate plots
fn_plot_q31 <- function(cols, labs) {
  (
    ggplot(
      data    = dt_q31,
      # mapping = aes(x = year, y = dt_q31[[cols]])
      mapping = aes(x = year, y = dt_q31[[cols]])
    ) +
      geom_line() +
      geom_smooth(formula = y ~ x, method = "loess", size = .5) +
      ...
      # facet_grid(toupper(exp) ~ type, scales = "free_y") +
      # labs(color = "Takeoff thrust") +
      theme_light() +
      theme(axis.title.y = element_blank())
  ) |>
  ggsave(
    filename = paste("9_q31_", as.name(cols), "_globally.png", sep = ""),
    device   = "png",
    path     = "plots",
    scale    = 1,
    width    = 6,
