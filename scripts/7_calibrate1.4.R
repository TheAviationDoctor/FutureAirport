...
fn_calibrate <- function(cD0_total, k_total, A, bpr, cD, cL, G0, hdw, n, rho, S, slst, Vlof, Vsnd, W, X, Z, todr_cal) {
  ...
  todr <- fn_todr(A, bpr, cD, cL, G0, hdw, n, rho, S, slst, Vlof, Vsnd, W, X, Z)
...
dt_tko[, cL := optimize(f = function(cL) fn_calibrate(cD0_total, k_total, A, bpr, cD, cL, G0, hdw, n, rho, S, slst, Vlof, Vsnd, W, X, Z, todr_cal),
                        interval = c(0.1, 1),
                        tol = .0015)$minimum,
       by = seq_len(nrow(dt_tko))]
