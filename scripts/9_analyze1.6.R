...
# Rename the first-year value columns
setnames(
  x   = dt_q37,
  old = c("i.avg_thr", "i.avg_pax"),
  new = c("avg_thr_ini", "avg_pax_ini")
)
...
dt_q37[, paste(c("avg_thr", "avg_pax", "avg_thr_inc", "avg_pax_inc"), "_loess", sep = "") :=
         lapply(X = .SD, FUN = function(x) {
           predict(loess(formula = x ~ year, span = .75, model = TRUE))
         }),
       by = c("exp", "zone", "type"),
       .SDcols = c("avg_thr", "avg_pax", "avg_thr_inc", "avg_pax_inc")
]
...
(
  ggplot(
    data    = dt_q37[type == "Narrowbody"],
    mapping = aes(x = year, y = avg_pax, color = zone)
  ) +
    # geom_line() +
    geom_smooth(formula = y ~ x, method = "loess", size = .5) +
    # geom_text(data = dt_q37[, .SD[which.min(year)], by = c("exp", "zone")], aes(x = year, y = avg_pax, color = zone, label = avg_pax), size = 2, vjust = 0) +
    geom_text(data = dt_q37[type == "Narrowbody"][, .SD[which.min(year)], by = c("exp", "zone")], aes(x = year, y = avg_pax_loess, color = zone, label = round(avg_pax_loess, digits = 1L)), size = 2L, vjust = -1L) +
    scale_x_continuous(name = "Year") +
