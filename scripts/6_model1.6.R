...
fn_todr <- function(DT) {
  ...
  q <- Vtas^2 * .5 * rep(DT[, rho], each = sim$int)
  ...
  Vmach <- Vtas / rep(DT[, Vsnd], each = sim$int)

  # Calculate the thrust ratio for each Mach number interval
  bpr_vec <- rep(DT[, bpr], each = sim$int)
  tr <- rep(DT[, A], each = sim$int) - .377 * (1 + bpr_vec) /
    sqrt((1 + .82 * bpr_vec) * rep(DT[, G0], each = sim$int)) *
    rep(DT[, Z], each = sim$int) * Vmach +
    (.23 + .19 * sqrt(bpr_vec)) * rep(DT[, X], each = sim$int) *
      Vmach^2

  # Calculate the maximum takeoff thrust in N for each airspeed interval
  Fmax <- tr * rep(DT[, slst], each = sim$int) * rep(DT[, n], each = sim$int)

  # Apply the maximum takeoff thrust reduction permissible
  Frto <- Fmax * (100L - rep(DT[, rto], each = sim$int)) / 100L
  ...
  W_vec <- rep(DT[, W], each = sim$int)

# FOR TESTING ONLY
L <- q * rep(DT[, S], each = sim$int) * rep(DT[, cL], each = sim$int)

# FOR TESTING ONLY
D <- q * rep(DT[, S], each = sim$int) * rep(DT[, cD], each = sim$int)

# Calculate the acceleration in m/s
a <- sim$g / W_vec * (Frto - (sim$mu * W_vec) -
                        (rep(DT[, cD], each = sim$int) - sim$mu * rep(DT[, cL], each = sim$int)) *
                        (q * rep(DT[, S], each = sim$int)) - (W_vec * sin(sim$theta)))

# FOR TESTING ONLY
a_new <- (Frto - D + sim$mu * L - W_vec * (sim$mu * cos(sim$theta) - sin(sim$theta))) / rep(DT[, m], each = sim$int)
  ...
  width <- rep(x = c(seq.int(2), rep(2, sim$int - 2)), times = nrow(DT))
