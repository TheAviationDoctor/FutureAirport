...
# Calculate the extended flap area
set(x = dt_act, j = "S_flaps", value = dt_act[, S_clean] * dt_act[, SfS])

# Calculate the total wing area with flaps extended
set(x = dt_act, j = "S_total", value = dt_act[, S_clean] + dt_act[, S_flaps])
...
db_qry <- paste(
  "SELECT DISTINCT icao FROM", tolower(db$pop),
...
fn_simulate <- function(icao) {
  ...
  db_qry <- paste(
    ...
    # " WHERE icao = '", icao, "' AND exp = 'ssp126';",
    sep = ""
  )
...
# dt_tko <- dt_tko[type == "A359", ]
# dt_tko <- dt_tko[obs == "2015-01-12 00:00:00", ]
# dt_tko <- head(dt_tko, n = 2868L)
...
# View(dt_tko[itr != 2, ])
  ...
  cols <- c(
    "obs", "icao", "lat", "lon", "exp", "type", "hurs", "ps", "tas", "rho",
    "hdw", "rwy", "toda", "todr", "Vlof", "rto", "rm", "cD", "cL", "itr"
  )
