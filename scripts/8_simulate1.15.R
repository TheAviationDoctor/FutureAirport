...
# Set the minimum calibrated mass below which the simulation should not run
dt_cal[dt_cal[, min(m), by = type], on = "type", mintom := V1][]
...
fn_simulate <- function(icao) {
  ...
  set(x = dt_tko, j = "m", value = dt_tko[, mintom] + 261L)
  ...
  set(x = dt_tko, j = "rto", value = 0L) # FOR TESTING ONLY
  ...
  repeat {
    ...
    # is greater than the minimum mass for which there is calibrated data
    ...
    if (length(i) > 0) {
      ...
      # Remove takeoffs where new mass could not be found in calibration data
      # (because the calibration data is filtered for min acceptable difference)
      # dt_tko <- na.omit(dt_tko, cols = "cD")
...
# FOR TESTING ONLY
print(dt_tko[i, mean(itr)])
print(dt_tko[i, mean(rto), by = type])
