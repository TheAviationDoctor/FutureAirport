...
# db_qry <- paste("SELECT type, m, cl, cd FROM ", tolower(db$cal),
db_qry <- paste("SELECT type, m, cl, cd, diff FROM ", tolower(db$cal), ";", sep = "")
                # " WHERE diff < ", sim$diff_thr, ";", sep = "")
...
# Find the minimum mass value that still falls within the calibration threshold
# We can't simply filter rows above the threshold, as the diff oscillates a bit
# and we would end up with a non-contiguous sequence of mass values.
dt_thr <- dt_cal[diff < sim$diff_thr, .SD[.N, m], by = type]

# Rename the new column to minimum takeoff mass
setnames(dt_thr, "V1", "mintom")

# Add the minimum takeoff mass to the calibration data
dt_cal <- dt_thr[dt_cal, on = c("type")]
...
# ==============================================================================
# 1.2.4 Excluding airports already processed (for incremental simulation runs)
# ==============================================================================
...
# FOR TESTING ONLY
set(x = dt_tko, j = "m", value = dt_tko[, maxtom] - 150 * 87L)
