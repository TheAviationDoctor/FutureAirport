...
fn_todr <- function(dt) {
  ...
  # dt[, t1 := Map("*", Vmach, 0.377 * (1 + bpr) / sqrt((1 + 0.82 * bpr) * G0) * Z)
  #    ][, t2 := Map("-", A, t1)
  #    ][, t3 := Map("^", Vmach, 2)
  #    ][, t4 := Map("*", t3, (0.23 + 0.19 * sqrt(bpr)) * X)
  #    ][, t5 := Map("+", t2, t4)]
  
  # # Calculate the maximum takeoff thrust in N for each Mach number
  # dt[, Fmax := Map("*", thrust_ratio, 1)]
  ...
  # # ]
  # 
  # dt[, a := Map(function(i) { sim$g / W[[i]] * (Frto[[i]] * 2 - (sim$mu * W[[i]]) - (cD[[i]] - (sim$mu * cL[[i]])) * (q[[i]] * S[[i]]) - (W[[i]] * sin(sim$theta))) }, seq_along(.I) )]
  # 
  # ##############################################################################
