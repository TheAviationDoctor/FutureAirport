...
dt_q37[, avg_thr_dlt := (avg_thr - i.avg_thr) / i.avg_thr][, i.avg_thr := NULL]
dt_q37[, avg_pax_dlt := (avg_pax - i.avg_pax) / i.avg_pax][, i.avg_pax := NULL]

# Perform local polynomial regression fitting to dampen the volatility
dt_q37[,
  paste(
    c("avg_thr", "avg_pax", "avg_thr_dlt", "avg_pax_dlt"),
    "loess",
    sep = "_"
  ) := lapply(
    X = .SD,
  ...
  by = c("exp", "zone", "type"),
  .SDcols = c("avg_thr", "avg_pax", "avg_thr_dlt", "avg_pax_dlt")
]
View(dt_q37)
stop()
...
fn_plot <- function(body, cols) {
  (
    ggplot(
      ...
      mapping = aes(
        x     = year,
        y     = dt_q37[type == body][[as.character(cols)]],
        color = zone
      )
    ) +
      geom_line(size = .2) +
      geom_smooth(formula = y ~ x, method = "loess", size = .5) +
      geom_text(
        data = dt_q37[type == body][, .SD[which.min(year)], by = c("exp", "zone")],
        aes(
          x     = year,
          y     = dt_q37[type == body][, .SD[which.min(year)], by = c("exp", "zone")][[paste(as.character(cols), "loess", sep = "_")]],
          color = zone,
          label = round(
            x = dt_q37[type == body][, .SD[which.min(year)], by = c("exp", "zone")][[paste(as.character(cols), "loess", sep = "_")]],
            digits = 1L
          )
        ),
        size  = 2L,
      ...
      geom_text(
        data = dt_q37[type == body][, .SD[which.max(year)], by = c("exp", "zone")],
        aes(
          x     = year,
          y     = dt_q37[type == body][, .SD[which.max(year)], by = c("exp", "zone")][[paste(as.character(cols), "loess", sep = "_")]],
          color = zone,
          label = round(
            x = dt_q37[type == body][, .SD[which.max(year)], by = c("exp", "zone")][[paste(as.character(cols), "loess", sep = "_")]],
            digits = 1L
          )
        ),
        size  = 2L,
      ...
      # geom_text(data = dt_q37[type == body][, .SD[which.min(year)], by = c("exp", "zone")], aes(x = year, y = avg_thr_loess, color = zone, label = 1L), size = 2L, vjust = -1L) +
      scale_x_continuous(name = "Year") +
  ...
  ggsave(
    filename = tolower(paste("9", body, cols, "loess.png", sep = "_")),
    device   = "png",
    path     = "plots",
    scale    = 1L,
    width    = 6L,
