...
fn_todr <- function(DT) {
  ...
  q <- Vtas * .5 * rep(DT[, rho], each = sim$int)
  ...
  tr <- rep(DT[, A], each = sim$int) - 0.377 * (1 + bpr_vec) /
    sqrt((1 + 0.82 * bpr_vec) * rep(DT[, G0], each = sim$int)) *
    rep(DT[, Z], each = sim$int) * Vmach +
    (0.23 + 0.19 * sqrt(bpr_vec)) * rep(DT[, X], each = sim$int) *
      Vmach^2
  ...
  Frto <- Fmax * (100 - rep(DT[, rto], each = sim$int)) / 100
