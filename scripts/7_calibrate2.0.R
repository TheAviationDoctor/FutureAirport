...
dt_act <- fread(
  file = f$act,
  header = TRUE,
  colClasses = c(rep("factor", 3), rep("integer", 3), rep("numeric", 9)),
  key = "type"
)
...
set(
  x = dt_tko,
  j = "cD0_total",
  value = dt_tko[, cD0] + dt_tko[, cD0_flaps] + dt_tko[, cD0_gear]
)

# ==============================================================================
# 1.6 Calculate the lift-induced drag coefficient factor k
# Adapted from from Sun et al. (2020).
# ==============================================================================
...
#   x = dt_tko,
#   j = "k_total",
#   value = 1 / (1 / dt_tko[, k] + pi * dt_tko[, ar] * dt_tko[, e_flap])
# )

# FOR TESTING ONLY
# dt_tko <- dt_tko[m == max(dt_tko[, m])]
...
fn_calibrate <- function(cL, i) {
  ...
  # FOR TESTING ONLY - LIFT-INDUCED DRAG
  set(
    x = dt_tko,
    i = i,
    j = "cDi",
  ...
  set(
    ...
    i = i,
    j = "cD",
    value = dt_tko[i, cD0_total] + dt_tko[i, cDi]
  )
