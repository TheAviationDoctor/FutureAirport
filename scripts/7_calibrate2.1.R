...
# ==============================================================================
# 1.5 Calculate the zero-lift drag coefficient cD0 in takeoff configuration
# Adapted from from Sun et al. (2020).
# ==============================================================================

# Calculate the zero-lift drag coefficient attributable to the flaps
# set(
#   x = dt_tko,
#   j = "cD0_flaps",
#   value = .9 * dt_tko[, cfc]^1.38 * dt_tko[, SfS] * sin(sim$flap_angle)^2
# )

# Calculate the zero-lift drag coefficient attributable to the landing gear
# set(
#   x = dt_tko,
#   j = "cD0_gear",
#   value = dt_tko[, m] * sim$g / dt_tko[, S] * 4.5E-5 * dt_tko[, m]^-.215
# )

# Calculate the total zero-lift drag coefficient
# set(
#   x = dt_tko,
#   j = "cD0",
...
# ==============================================================================
# 1.6 Calculate the lift-induced drag coefficient k in takeoff configuration
# Adapted from from Sun et al. (2020).
# ==============================================================================
...
# Calculate the Oswald efficiency factor attributable to the flap setting
set(x = dt_tko, j = "e_flap", value = .0026 * sim$flap_angle)

# Calculate the lift-induced coefficient k in non-clean configuration
set(
  x = dt_tko,
  j = "k",
  value = 1 / (pi * dt_tko[, ar] * (dt_tko[, e_clean] + dt_tko[, e_flap]))
  # value = 1 / (pi * dt_tko[, ar] * dt_tko[, e_clean])
)
...
fn_calibrate <- function(cL, i) {
  ...
  # Calculate the lift-induced drag coefficient # METHOD 1
  # set(
  #   x = dt_tko,
  #   i = i,
  #   j = "cDi",
  #   value = cL^2 / (pi * dt_tko[i, ar] * dt_tko[i, e])
  # )

  # Calculate the lift-induced drag coefficient # METHOD 2
  set(
    x = dt_tko,
    i = i,
    j = "cDi",
    value = dt_tko[i, k] * cL^2
    # value = dt_tko[i, k_clean] * cL^2
  )
...
for (i in seq_len(nrow(dt_tko))) {
  ...
  # ifelse(
  #   i == 1,
  #   lower <- min(sim$clmax_range),
  #   lower <- dt_tko[i-1, cL] * .99
  # )
  ...
  optimize(
    ...
    # interval = c(lower, max(sim$clmax_range)), # FOR TESTING ONLY
    tol = sim$optim_tol
  )
