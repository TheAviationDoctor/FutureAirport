...
fn_wnd <- function(nc_exp) {
  ...
  # Set the names of the tables where the eastward and northward wind data are stored
  ...
  # Build the query to retrieve the wind data for every airport/observation pair
  db_qry <- paste("SELECT uas.obs, uas.icao, uas.val AS uas, vas.val AS vas FROM", db_uas, "AS uas,", db_vas, "AS vas WHERE uas.id = vas.id ORDER BY uas.icao, uas.obs;", sep = " ")
